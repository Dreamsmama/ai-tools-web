...
面试展示价值在于：你可以完整讲清楚一个 AI 功能如何从“模型调用”进化成“可运营、可观测、可扩展”的后端系统。
...
- 为什么要做文档状态机，而不是一次性“上传即问答”；
- 为什么要记录 retrieval/generation/total 三段耗时；
- 为什么先做向量检索抽象层，而不是把 pgvector 逻辑写死在业务代码里；
- 如何从当前架构平滑升级到“混合检索 + 重排 + 缓存”；
- 如何在接入真实用户体系后做更细粒度权限模型（workspace、official/user kb、发布审核）。

---

## 8) 目录结构优化建议（保持简单，不做过度设计）

当前仓库同时包含小程序与 Web 端能力。建议在不大改代码的前提下做一层分组，提升“面试可读性”：
...
这些点能显著提升“工程化成熟度”观感，是面试官判断候选人上限的关键信号。
