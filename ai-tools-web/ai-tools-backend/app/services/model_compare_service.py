...
async def call_original_model(input_text: str) -> str:
    ...
    return await call_dashscope([system_message, user_message])


async def call_optimized_model(input_text: str) -> str:
    optimized_model = settings.dashscope_optimized_model.strip()
    if optimized_model:
        raw = await call_dashscope(_build_structured_prompt(input_text), model=optimized_model)
        return _normalize_optimized_output(raw)

    raw = await call_dashscope(_build_structured_prompt(input_text))
    return _normalize_optimized_output(raw)
